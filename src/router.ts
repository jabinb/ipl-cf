...
const handleStaticFile =
  ...
  () =>
    env.CLIENT_CONTENT.get(fileName)
      ...
      .catch(
        (e: Error) =>
          new Response(`Error handling static file response: ${e.message}`, {
            status: 500,
          }),
      );
...
const handleHashesContent =
  ...
  () =>
    getHashes(env, hashesFileName).then(
      (m) => new Response(hashesToString(m), { status: 200 }),
    );
...
const handleManifestContent =
  ...
  async () => {
    ...
    const manifestVersion =
      (await env.CONTENT_KV.get(ManifestVersionName)) ?? "0.0.0.1";
...
const handleContentFile =
  ...
  async (request) => {
    ...
    const manifestFile = manifest.find(
      (f) => f.toLowerCase() === filePath.toLowerCase(),
    );
